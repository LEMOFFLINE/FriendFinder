...
export async function POST(request: NextRequest, { params }: { params: Promise<{ postId: string }> }) {
  try {
    ...
    // Get original post
    const originalPost = await query("SELECT * FROM posts WHERE post_id = $1 AND is_deleted = false", [originalPostId])
    ...
    await query("UPDATE posts SET repost_count = repost_count + 1 WHERE post_id = $1", [originalPostId])
