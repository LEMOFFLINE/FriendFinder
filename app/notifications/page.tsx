...
export default function NotificationsPage() {
  ...
  const handleBlock = (request: any) => {
    if (!user) return

    const confirmed = confirm(`Block ${request.fromUserName}? They won't be able to send you friend requests.`)
    if (!confirmed) return
    ...
    if (requesterData && !blocked.find((b: any) => b.id === requesterData.id)) {
      blocked.push(requesterData)
      localStorage.setItem(`blocked_${user.id}`, JSON.stringify(blocked))
    }

    const updatedRequest = { ...request, status: "blocked" }
    const allReceivedRequests = JSON.parse(localStorage.getItem(`friend_requests_received_${user.id}`) || "[]")
    const updatedReceivedRequests = allReceivedRequests.map((r: any) => (r.id === request.id ? updatedRequest : r))
    localStorage.setItem(`friend_requests_received_${user.id}`, JSON.stringify(updatedReceivedRequests))
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-3 flex gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 rounded-lg border border-red-200 px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors"
                      >
                        Block
                      </button>
                    </div>
